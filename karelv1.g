...
struct {
    ...
    //vector<i3tuple>  walls;
    //vector<i3tuple>  beepers;
...
bool isClear(int x, int y, int orient) {
    ...
    return dinsDominis(next_x, next_y) and domini.mapa[x][y].hasWall_i(orient) and domini.mapa[next_x][next_y].hasWall_i(next_orient);
}

//PRE: el moviment es correcte
void move(){
...
void execute(AST *a){
    if (a == NULL) return ;
        ...
        if (isClear(domini.posx, domini.posy, domini.orient)) move();
    }
...
void setWallsRec(AST *a){
    if (a == NULL) return ;
        ...
        cout << "wall: " << a->kind.c_str() << " " << a->right->kind.c_str() << " " << a->right->right->kind.c_str() << endl;
...
void setDefinitions(AST *a){
    ...
    if (a == NULL) return ;
        ...
        cout << "beeper: " << x << " " << y << " " << beepers << endl;
    }
    /*
  	else if (a->kind == "define") {
      mdefines[a->down->text.c_str()] = a->down->right->down;
...
void omplirDomini() {
    ...
    domini.mapa = vector<vector<Casilla> > (domini.gx, vector<Casilla> (domini.gy));
...
#token ID "[a-zA-z0-9]*"
   ...
   atominst: (MOVE ENDINST!)|(TURNLEFT ENDINST!)|(PUTBEEPER ENDINST!)|(PICKBEEPER ENDINST!)|si|iterations|(ID ENDINST!)|(TURNOFF ENDINST!);
